...
values ('openroad-vehicle-images', 'openroad-vehicle-images', true)
on conflict (id) do update set public = true;

drop policy if exists "openroad storage public read" on storage.objects;
drop policy if exists "openroad storage admin insert" on storage.objects;
drop policy if exists "openroad storage admin update" on storage.objects;
drop policy if exists "openroad storage admin delete" on storage.objects;

create policy "openroad storage public read"
  on storage.objects for select
  to anon, authenticated
  using (bucket_id = 'openroad-vehicle-images');

create policy "openroad storage admin insert"
  on storage.objects for insert
  to authenticated
  with check (bucket_id = 'openroad-vehicle-images');

create policy "openroad storage admin update"
  on storage.objects for update
  to authenticated
  using (bucket_id = 'openroad-vehicle-images')
  with check (bucket_id = 'openroad-vehicle-images');

create policy "openroad storage admin delete"
